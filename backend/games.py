...
class Game:
    def __init__(self, num_ai):
        ...
        self.history = []        # Store history of rounds
    ...
    def win(self):
        ...
        # --- Add to history after each round ---
        round_data = {
    ...
    def serialize(self):
        return {
            ...
            'twist_num': self.twist_num,
            'player_alive_num': self.player_alive_num,
            'duplicate_guesses': self.duplicate_guesses
        }
    ...
    def play_move(self, move):
        # Accepts a guess from the human player, processes a round, and returns the updated state/result
        # Find the human player
        human = next((p for p in self.players if not p.is_ai and p.alive), None)
        if human is None:
            return {'error': 'No alive human player'}
        try:
            guess = int(move)
        except (ValueError, TypeError):
            return {'error': 'Invalid move'}
        if not (0 <= guess <= 100):
            return {'error': 'Guess must be between 0 and 100'}
        human.guess = guess
        # AI players make their guesses
        for p in self.players:
            if p.is_ai and p.alive:
                p.guess = random.randint(0, 100)
        # Continue the round as in play(), but only for one round
        self.duplicate_guesses = []
        if self.player_alive_num >= 4:
            for player in self.players:
                if player.alive:
                    if player.guess in [pl.guess for pl in self.players if pl.alive and pl != player]:
                        self.duplicate_guesses.append(player.guess)
        self.twist()
        self.win()
        self.die()
        self.about()  # Optionally print status to console
        self.rounds()
        self.sum = 0
        self.twist_num = 0
